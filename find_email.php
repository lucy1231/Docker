...
이메일의 가입 여부를 알려줍니다.
<form name="find_email" method="POST" action="find_email.php">
<input placeholder="조회할 이메일?" type="text" name="email">
...
$check=mysql_fetch_array(mysql_query("select * from user where email = '".$email."';"));
if($email && $check) /*조회 버튼을 눌러서 이메일을 보냈고 확인이 된 경우*/
{
	echo"<br>가입되어 있는 이메일입니다";
}else if($email) /*조회 버튼을 눌러서 이메일을 보냈으나 체크가 없는경우*/
	echo"<br>가입 되어 있지 않습니다.";
